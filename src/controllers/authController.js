...
const register = async (req, res) => {
  try {
    ...
    const passwordHash = await bcrypt.hash(password + process.env.EXTRA_BCRYPT_STRING, 10);
...
const login = async (req, res) => {
  try {
    ...
    const match = await bcrypt.compare(password + process.env.EXTRA_BCRYPT_STRING, user.passwordHash);
    if (!match) {
      return res.status(401).json({ message: 'Invalid credentials' });
    }
